...
public abstract class CameraAbstract : MonoBehaviour {
    ...
    protected virtual void Awake() {
        string controlType = PlayerPrefs.GetString("ControlType");
        if (controlType == "Mobile")
        {
            mobileVersion = true;
        }
        else if (controlType == "PC")
        {
            mobileVersion = false;
        }
        ...
        transform.parent = observedObject; // Unparent the camera itself so it can move freely and use the target to lerp smoothly
    ...
    */
        ...
        if (Input.touchCount > 0)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Moved:
                    ...
                    float rotationAngleX = -currentRotation.y * XSensitivity;
                    float rotationAngleY = currentRotation.x * YSensitivity;

                    // Вычисляем новый угол вращения по осям X и Y
                    float newXRotation = transform.eulerAngles.x + rotationAngleX / 10;
                    float newYRotation = transform.eulerAngles.y + rotationAngleY / 10;
                    ...
                    if (isRotating)
                    {
                        ...
                        newXRotation = Mathf.Clamp(newXRotation, -camLowerAngleMargin, camUpperAngleMargin);

                        // Применяем новый угол вращения
                        Quaternion newRotation = Quaternion.Euler(newXRotation, newYRotation, 0);
                        transform.position = observedObject.position - newRotation * Vector3.forward * Vector3.Distance(transform.position, observedObject.position);
    ...
    public void SetDistanceCamera(float value)
    {
